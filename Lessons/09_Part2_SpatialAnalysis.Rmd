...
ggplot()+
  ...
  geom_sf(data = huc8_sf_sub, color = "blue", fill = "yellow")
...
huc_12 <- st_read('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv')
huc_12$HUC_8 <- substr(huc_12$HUC_12, start = 1, stop = 8)

huc_12$HUC_8 <- as.factor(huc_12$HUC_8)

huc_12$AvgWindEnergy <- as.numeric(huc_12$AvgWindEnergy)
huc_12$kWhkm2day <- as.numeric(huc_12$kWhkm2day)


huc_12_group <- huc_12|>
  select(HUC_8, AvgWindEnergy, kWhkm2day)|>
  group_by(HUC_8)|>
  summarise(
    total_wind = sum(AvgWindEnergy),
    total_kwh = sum(kWhkm2day)
  )
...
huc_8_complete <- left_join(huc8_sf, huc_12_group, by = c("HUC_8" = "HUC_8"))

huc_8_sum_wind <- huc_8_combo |>
  summarize(
    total_wind_energy = sum(AvgWindEnergy,na.rm=T)
    )

#View the outputs
ggplot() + 
  geom_sf(data=huc_8_combo,aes(fill = AvgWindEnergy),alpha=0.5) + 
  scale_fill_gradient2(low="red",high="green",midpoint = 0.5) 
...
huc2_sf <- huc_8_combo |>
  ...
  summarize(
    total_wind_energy = sum(AvgWindEnergy,na.rm=T),
    total_kwh = sum(kWhkm2day,na.rm=T),
...
huc_8_complete_utm <-        st_transform(huc_8_complete, crs = 26917)
```
...
triangle_intersect <- triCo |>  
  st_filter(huc_8_complete_utm, .predicate = st_intersects)

myMap1 <- ggplot(data = huc_8_complete_utm)+
  geom_sf()
myMap1

#View the result
myMap1 + geom_sf(data = triangle_intersect, color = "red")


#Intersect the HUC_8s
triCo_counties_sf <- huc_8_complete_utm |> st_intersection(triCo)

#View the result
ggplot()+
  geom_sf(data = triCo_counties_sf)
```
