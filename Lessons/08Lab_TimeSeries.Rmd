...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% # similar to text to column in excel. " " is seperator 
...
# inner join helps make sure no data is missing
...
ggplot(BR_complete)+
  geom_line(aes(Date, Rainfall))+
  geom_smooth(aes(Date, Rainfall),
              method = lm)

ggplot(BR_complete)+
  geom_line(aes(Date, Temperature_C))+
  geom_smooth(aes(Date, Temperature_C),
              method = lm)
...
# s.window means seasonal window, and it is periodic
...
Kendall::SeasonalMannKendall(BR_rainfall_ts)

Kendall::SeasonalMannKendall(BR_temp_ts)
