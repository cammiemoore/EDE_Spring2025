...
today <- Sys.Date()
format(today, format = "%B") #full name of month %b = shortened name of month
#format(today, format = "")
#format(today, format = "")
#format(today, format = "")
...
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE) # shows all unique data even if there is no data
...
usgs_flow_data$datetime = fast_strptime(usgs_flow_data$datetime,"%m/%d/%Y",cutoff_2000=24L)
#Create date column
