...
O3plot2 <- ggplot(EPAair) +
  ...
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2) # specificing color in aes is like adding a layer or identifying a variable
# fixed

# Add additional variables
# How could you automatically assign a marker color to a variable? 
# asking r to assign a different shape to a factor. Since it is inside the aes it applies it like a layer and automatically creates a legend. Color is inside the aes but is specified by site name.
PMplot <- 
...
# facet wrap adds plots based on a factor variable
PMplot.faceted <-
...
# Filter dataset within plot building and facet by multiple variables. Selecting only sites you want using subset function.
PMplot.faceted2 <- 
...
# Plot true time series with geom_line. Connects points.
PMplot.line <- 
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) + #finding the trend line
  geom_abline(aes(slope = 16, intercept = 0)) #specifies the trend line
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Fill changes the fill of boxplot not the dot colors.
print(Nutrientplot3)
...
print(Nutrientplot4)
# binaxis means stacking over y. Binwidth, number of obs. stackdir means stacking over center. Position dodge prevents the circles from overlaying, and gives a visualization of number of obs at a certain concentration.
...
print(Nutrientplot6)
# no legend, not a good way to generate these plots. Much better way is to use the gathered dataset.
