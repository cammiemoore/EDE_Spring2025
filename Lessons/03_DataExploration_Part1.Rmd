...
> Answer: The console unpacks and installs the package.
...
# We put "strings as factors" so all chr are immediatedly treated as factors.

# What happens if we don't assign a name to our imported dataset? It gets displayed in the console and not saved as an object.
...
> ANSWER: Data was collected from USGS. The file can be found via USGS website. The information provided in the file relates to best practices because it clearly details where the data was collected, who collected it, when the data was collected, how the data was collected, and is a complete data set. 
...
str(USGS.flow.data) # great way to see the data types
dim(USGS.flow.data) # dimension of data frame
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) # fairly brutal way to get rid of missing data
dim(USGS.flow.data)
...
```{r Fixing dates}
# changing the date format from m/d/y to y/m/d - which is better for international use, but there is no seperators
...
# using the create early dates function to parse the dates
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

# formatting the dates into four year dates
