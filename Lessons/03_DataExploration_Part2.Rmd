...
>Answer: We can't make inferences or gather evidence from data without understanding it first. I feel like exploring is probably one of the more important part of data exploration 
...
# Unique command or levels shows factor categories
levels(USGS.flow.data$discharge.mean.approval)
```
...
```{r, fig.height = 3, fig.width = 4}
# 
ggplot(USGS.flow.data) + # graph paper
  geom_histogram(aes(x = discharge.mean)) # type of graph, aes is for data only
...
```{r, fig.height = 3, fig.width = 4}
# this graph shows values of data, 
ggplot(USGS.flow.data) + 
  geom_freqpoly(aes(x = gage.height.mean), bins = 50) +
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "darkgray") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 2) + # lty = line type, go to help, data visualization, ggplot cheat sheet
  scale_x_continuous(limits = c(0, 10)),

#
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean, color = gage.height.mean.approval), bins = 50) + # makes the color a categorical value
  scale_x_continuous(limits = c(0, 10)) +
  theme(legend.position = "top") # add a legend
...
#
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean)) # you can make a categorical variable to have side by side box plots

#
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1))) # box plot of two continuous values

#
ggplot(USGS.flow.data) +
  geom_violin(aes(x = gage.height.mean.approval, y = gage.height.mean), 
              draw_quantiles = c(0.25, 0.5, 0.75)) # width is proportional to number of values in the range. Marks quartile, median, and lower quartile
```
...
> Answer: Bar plot is beneficial for comparing levels within categorical data. A histogram is great for seeing data distributions. A frequency polygon is excellent for analyzing multiple data distributions in the same graph. A box and whisker gives us a 5 number summary quickly and easily - similar to a violin plot. A scatterplot is great for comparing two continuous variables.

## Ending discussion
What did you learn about the USGS discharge dataset today? What separate insights did the different graph types offer?
> Answer: Some graphs showed data distributions and patterns. For example, the frequency polygons showed the counts of mean, min, and max discharges. Additionally, it would be difficult to summarize the data without the boxplots quickly synthesizing hundreds of data points. The scatterplot was great for finding an association between continuous variables.


How can multiple options for data exploration inform our understanding of our data?

> Answer: Multiple options for data exploration allows us to examine the data from all angles. It helps us better understand the data in completeness.
...
> Answer: The avereage discharge rises with the average gage height. This association makes sense considering the more discharge means more water which will raise the gage.
