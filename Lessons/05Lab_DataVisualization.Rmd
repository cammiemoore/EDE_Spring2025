...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    ...
    ) # expression allows you to use greek symbols and subscript. paste allows for multiple objects seperated by commas.
...
PeterPaul.chem.nutrients |>
  ggplot()+
  geom_bar(aes(x=factor(month, levels=1:12, labels = month.abb)))+
  scale_x_discrete(drop=F, name = "Month") # do not drop levels which have no observations

#Tidy up the code
PeterPaul.chem.nutrients |>
  ggplot()+
  geom_bar(aes(x=factor(month, levels=1:12, 
                        labels = month.abb)))+
