...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #
  mutate(month = month(sampledate)) %>% # creates month column
  select(lakeid:daynum, month, sampledate:comments) %>% # rearranging so month is near other dates
  drop_na(depth)
...
# Spread nutrient data into separate columns. Turning rows into columns
