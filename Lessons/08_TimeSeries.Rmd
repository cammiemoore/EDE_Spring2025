...
Decomposing is showing seasonality/variability of different components. 
...
Note: time series objects must be equispaced (equally time-spaced, no NAs). In our case, we have daily data with no NAs in the data frame, so we don't need to worry about this. We will cover how to address data that are not equispaced later in the lesson.
...
The smaller the bar, the greater the effect on the variability of the data. Data is original data. Seasonality, trend, and random. Random is whatever does not fit into seasonality or trend.
