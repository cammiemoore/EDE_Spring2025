...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_046"]) #nor
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_057"]) #nor
...
# structure: data = dataframe, continuous ~ factor
# Format ANOVA as aov
...
# above is showing results with respect to intercept statistic chosen
...
# takes argument which should be result of AOVA test
...
# results with the same letter have the same mean
...
> We would want to organize the graph so that all the letters are organized in a way that is easily comparable.
...
```{r}
# Format as aov, use * to write about interaction
Litter.anova.2way3 <- aov(data = Litter, dryMass ~ functionalGroup * nlcdClass)
summary(Litter.anova.2way3)

# if interaction is significant, we interpret pairwise differences for the interaction
...
Litter$litter_groups <- Litter

# Plot the results
# Edit this graph to make it more visually appealing.
# Add letters to represent statistical groupings.
Litter.anova.plot <- ggplot(Litter.groups, aes(y = dryMass, x = nlcdClass, color = functionalGroup)) +
  geom_boxplot()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5,
               label = c("a", "b", "bc", "cd", "d")) +
