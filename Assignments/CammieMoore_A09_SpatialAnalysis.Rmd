...
gage.joined.sf <- right_join(gage.sf, gage.heights.df, by = "station_nm")
...
saunders.gage.utm <- saunders.utm |>  
  st_filter(gage.sf.utm, .predicate = st_intersects)
...
saunders.15gage.utm <- saunders.buffer.utm |>
  st_filter(gage.sf.utm, .predicate = st_intersects)

#25 Plot
ggplot()+
  geom_sf(data = nebraska.sf.utm)+
  geom_sf(data = saunders.gage.utm)+
  geom_sf(data = saunders.15gage.utm)
